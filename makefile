...
ifeq ($(wildcard ~/.opvdm_hpc_flag),)
        ...
        link+= -L/users/sg/rod/libzip/libzip-0.11.2/install/usr/local/lib/ -L/usr/lib/umfpack
        debug_opt=
endif
...
main: main.c solver.o light_utils.o gui_hooks.o sim_find_n0.o sim_run.o newton_update.o  pos.o inital.o advmath.o config.o plot.o memory.o dos.o gendosfdgaus.o exp.o time.o fast.o anal.o dump.o ntricks.o dos_an.o startstop.o render1d.o complex_solver.o dump_slice.o thermal.o light_interface.o dump_ctrl.o light_dump.o light_test.o inp.o buffer.o
	...
	gcc main.c light_dump.o buffer.o light_utils.o light_test.o solver.o gui_hooks.o sim_find_n0.o sim_run.o newton_update.o pos.o inital.o advmath.o config.o plot.o memory.o dos.o gendosfdgaus.o exp.o time.o $(plugins) fast.o anal.o dump.o ntricks.o dos_an.o startstop.o render1d.o complex_solver.o dump_slice.o thermal.o light_interface.o dump_ctrl.o inp.o -o go.o  -L/usr/lib/umfpack/UMFPACK/Lib $(link) -lumfpack $(flags) $(inc) -lamd -Wall -lzip -export-dynamic -lm  -lgsl -lgslcblas -lblas -lcrypto -lcurl   -ldl -rdynamic $(opt_normal) $(llink)
